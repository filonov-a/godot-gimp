...
(define (export-png-withflags img drawable pngname mipmap)
     (if (= 1 mipmap)
	  (let ((outport
		 (open-output-file (string-append pngname ".flags" ))))
	    (display "gen_mipmaps=false\n" outport)
	    (close-output-port outport)))
     (file-png-save-defaults 1 img drawable pngname pngname))
;; ----------------------------------------------------------------
(define (write-node  outport . slist)
  (define (write-item   s)
    (display s outport))
  (for-each write-item slist ))
;; ----------------------------------------------------------------
(define (write-string-to-file  fname . slist)
...
(define (write-scene-header outport resname)
  ...
  (write-node  outport 
	       ...
	       resname 
	       ".png\" type=\"Texture\" id=1]\n\n" )
  (write-node  outport "[node name=\"Node2D\" type=\"Node2D\"]\n\n" ))


;; ----------------------------------------------------------------
(define (godot-create-tscn 
	 img drawable w h off_x off_y prefix dirname resname  mipmap )
  (let * (
    ...
    (gimp-message (string-append 
		   ...
		   " and scene to " tname ))))
;; ----------------------------------------------------------------
(define (godot-create-sprites 
	 img drawable direction  prefix dirname resname mipmap close_image)
  (let * ((tname (string-append  dirname DIR-SEPARATOR  resname ".tscn"))
    ...
    (gimp-message
     (string-append "Writing png to " pngname " and scene to " tname ))))
;; ----------------------------------------------------------------
(define (godot-split-layers img drawable prefix dirname resname)
  (let * ((tname (string-append  dirname DIR-SEPARATOR  resname ".tscn"))
	  (pngname "")
	  (out_image 0)
    ...
    ((do (( i 0 (+ 1 i)))
	 (( >= i numlayers))
       ...
       (set! pngname
	     (string-append
	      dirname DIR-SEPARATOR resname (number->string i) ".png"))
       (export-png-withflags img layer pngname  0)
       ;(gimp-message (string-append "Writing png to " pngname ))
       ))))
;; ----------------------------------------------------------------
...
(script-fu-register "godot-split-layers"
		    _"<Toolbox>/Xtns/Godot/Split Layers to separate images..."
		    "Split layers into separate image file, and generate .tcsn       "
		    "Andrew Filonov <andrew.filonov@gmail.com>"
		    "andrew.filonov@gmail.com"
		    "2016"
		    ""
